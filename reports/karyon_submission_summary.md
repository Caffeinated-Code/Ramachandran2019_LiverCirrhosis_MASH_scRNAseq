...
The repository is organized as an analysis pipeline rather than a notebook dump: `Makefile`, `renv.lock`, `Dockerfile`, `config/project.yaml`, `workflow/`, `scripts/`, `nextflow/`, `reports/`, and `dashboard/` each have a clear role. Raw data, large Seurat objects, validation matrices, private notes, and the assignment PDF are excluded from Git.

## What Was Done

The workflow starts from public GEO count matrices rather than relying on a hidden processed object. It curates GSE136103 metadata, excludes blood and mouse samples from the primary human liver contrast, builds Seurat objects, applies QC, normalizes, clusters, and generates UMAPs. Required disease-relevant compartments were recovered and marker-validated:
...
The published Ramachandran Seurat annotation object was then used as a reference layer. The workflow reads its `annotation_lineage` and `annotation_indepth` metadata and writes refined cluster labels without using the reference object as a shortcut for the primary analysis.
...
Candidate ranking uses a transparent rule-based score rather than a black-box model. The score considers disease association, compartment specificity, donor-aware support, pathway support, external validation, protein modality, mouse conservation, tractability, and safety or specificity risk.

The strongest near-term translational candidates are not all therapeutic targets:
